...
func (m *PostgresDBRepo) GetUserByEmail(email string) (*models.User, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, email).Scan(&user.ID, &user.Email, &user.FirstName, &user.LastName, &user.Password, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		//if err == sql.ErrNoRows {
		//	return nil, err
		//}
...
func (m *PostgresDBRepo) GetUserByID(id int) (*models.User, error) {
	...
	var user models.User
	err := m.DB.QueryRowContext(ctx, query, id).Scan(&user.ID, &user.Email, &user.FirstName, &user.LastName, &user.Password, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
